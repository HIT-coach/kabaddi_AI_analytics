...
def create_gradient_background(width, height):
    ...
    for i in range(width):
        # Calculate the grey intensity based on the column index
        grey_value = int(
            150 + (i / width * 1.5) * (255 - 150)
        )  # 169 is grey, 255 is white
...
def detect_and_track(video_path, output_path=None):
    ...
    # Track detection states and timers
    ...
    while cap.isOpened():
        ...
        # Reset detection flags for current frame
        ...
        if len(points) == 4:
            ...
            # Draw court boundary and center line
            for p1, p2 in [
            ...
            if top_mid and bottom_mid:
                cv2.line(frame, top_mid, bottom_mid, (0, 255, 0), 2)
            ...
            for team in ["team_1", "team_2"]:
                for player_point, bbox, confidence, track_id, duration in players[team]:
                    ...
                    cv2.rectangle(frame, (x1, y1), (x2, y2), colors[team], 2)

                    # Draw metrics above player's head
                    draw_player_metrics(
                        frame,
                        x1,
                        y1,
        ...
        # Save and overlay graphs only for detected teams
        active_graphs = []
        if team1_detected:
            save_graph_as_image(
